...
namespace GeekShopping.Web.Services
{
    public class CartService : ICartService
    {
        ...
        public Task<bool> ApplyCoupon(CartViewModel card, string couponCode, string token)
        {
        ...
        public async Task<bool> RemoveFromCart(long cardId, string token)
        {
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _client.DeleteAsync($"{BasePath}/remove-cart/{cardId}");
            if (response.IsSuccessStatusCode)
